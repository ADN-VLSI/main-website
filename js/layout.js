async function injectSharedLayout() {
  ...
  const response = await fetch(`partials/layout.html?v=${layoutVersion}`);
  if (!response.ok) {
...
function setupReactiveBackground() {
  ...
  let ambientDrift = 0;
  const lanesPerDirection = 4;
  const laneSpacing = 2.6;
  ...
  function makePacket(preferredSource) {
    ...
    return {
      ...
      laneSlot: randomInt(lanesPerDirection),
      segment: 0,
  ...
  function buildNoCMatrixModel() {
    ...
    const packetCount = Math.max(
      64,
      Math.min(220, Math.round(rows * cols * lanesPerDirection * 1.35))
    );
  ...
  function updatePointer(event) {
    ...
    if (event.target instanceof Element) {
      const isInteractive = event.target.closest(
        "a, button, input, textarea, select, label, summary, [role='button'], .btn, .hero-slider-dot"
      );
      document.body.classList.toggle("reactive-cursor-hover", Boolean(isInteractive));
    }
  }

  function hidePointer() {
    pointer.active = false;
    document.body.classList.remove("reactive-cursor-hover", "reactive-cursor-active");
  }
  ...
  function draw(time) {
    ...
    links.forEach((link) => {
      ...
      for (let lane = 0; lane < lanesPerDirection; lane += 1) {
        const plusOffset = (lane + 0.5) * laneSpacing;
        const minusOffset = -plusOffset;

        ctx.beginPath();
        ctx.strokeStyle = "rgba(155, 236, 90, 0.045)";
        ctx.lineWidth = 0.55;
        ctx.moveTo(
          link.from.x + normalX * plusOffset,
          link.from.y + normalY * plusOffset
        );
        ctx.lineTo(
          link.to.x + normalX * plusOffset,
          link.to.y + normalY * plusOffset
        );
        ctx.stroke();
        ...
        ctx.lineWidth = 0.55;
        ctx.moveTo(
          link.from.x + normalX * minusOffset,
          link.from.y + normalY * minusOffset
        );
        ctx.lineTo(
          link.to.x + normalX * minusOffset,
          link.to.y + normalY * minusOffset
        );
        ctx.stroke();
      }
    });
    ...
    tiles.forEach((tile) => {
      ...
      const alpha = isHot ? 0.66 : 0.34 + pulse;
      const size = isHot ? 36 : 30;
      const x = tile.x - size * 0.5;
      const y = tile.y - size * 0.5;
      const pinLen = 7.2;
      const laneOffsets = [];
      for (let lane = 0; lane < lanesPerDirection; lane += 1) {
        const laneOffset = (lane + 0.5) * laneSpacing;
        laneOffsets.push(-laneOffset, laneOffset);
      }
      laneOffsets.sort((a, b) => a - b);

      const bodyGradient = ctx.createLinearGradient(x, y, x + size, y + size);
      bodyGradient.addColorStop(0, `rgba(18, 34, 22, ${Math.max(0.22, alpha * 0.5)})`);
      bodyGradient.addColorStop(1, `rgba(30, 56, 34, ${Math.max(0.2, alpha * 0.46)})`);
      ctx.fillStyle = bodyGradient;
      ctx.fillRect(x, y, size, size);

      ctx.strokeStyle = `rgba(176, 255, 118, ${Math.max(0.16, alpha * 0.42)})`;
      ctx.lineWidth = 2.0;
      ctx.strokeRect(x - 0.5, y - 0.5, size + 1, size + 1);

      const dieSize = size * 0.42;
      const dieX = tile.x - dieSize * 0.5;
      const dieY = tile.y - dieSize * 0.5;
      ctx.fillStyle = `rgba(152, 243, 86, ${Math.max(0.15, alpha * 0.45)})`;
      ctx.fillRect(dieX, dieY, dieSize, dieSize);

      ctx.strokeStyle = `rgba(205, 255, 163, ${Math.max(0.12, alpha * 0.35)})`;
      ctx.lineWidth = 0.7;
      ctx.strokeRect(dieX - 0.4, dieY - 0.4, dieSize + 0.8, dieSize + 0.8);

      ctx.strokeStyle = `rgba(154, 234, 92, ${Math.max(0.1, alpha * 0.32)})`;
      ctx.lineWidth = 2.0;
      laneOffsets.forEach((offset) => {
        ctx.beginPath();
        ctx.moveTo(tile.x + offset, y);
        ctx.lineTo(tile.x + offset, y - pinLen);
        ctx.stroke();

        ctx.beginPath();
        ctx.moveTo(tile.x + offset, y + size);
        ctx.lineTo(tile.x + offset, y + size + pinLen);
        ctx.stroke();

        ctx.beginPath();
        ctx.moveTo(x, tile.y + offset);
        ctx.lineTo(x - pinLen, tile.y + offset);
        ctx.stroke();

        ctx.beginPath();
        ctx.moveTo(x + size, tile.y + offset);
        ctx.lineTo(x + size + pinLen, tile.y + offset);
        ctx.stroke();
      });
    });

    const dt = 0.016;
    spawnAccumulator += dt * (pointer.active ? 9.2 : 4.8);
    ...
    const maxPackets = Math.max(
      96,
      Math.min(320, Math.round(rows * cols * lanesPerDirection * 2))
    );
    ...
    packets.forEach((packet, packetIndex) => {
      ...
      const speed = packet.speed * pointerFactor * dt * 0.8;
      ...
      const laneOffset = packet.direction * (packet.laneSlot + 0.5) * laneSpacing;
      const px = x + normalX * laneOffset;
      const py = y + normalY * laneOffset;
      const agePulse = 0.75 + Math.sin(t * 3.2 + packet.phase) * 0.25;
      const alpha = packet.glow * agePulse * 0.3;
      const packetSize = packet.size;

      ctx.fillStyle = `rgba(201, 255, 153, ${alpha})`;
      ctx.fillRect(px - packetSize, py - packetSize, packetSize * 2, packetSize * 2);

      ctx.strokeStyle = `rgba(146, 255, 85, ${alpha * 0.4})`;
      ctx.lineWidth = 1;
      ctx.strokeRect(
        px - packetSize * 1.5,
        py - packetSize * 1.5,
  ...
  window.addEventListener("pointerdown", () => {
    document.body.classList.add("reactive-cursor-active");
  });
  window.addEventListener("pointerup", () => {
    document.body.classList.remove("reactive-cursor-active");
  });
...
function enforceNonBreakingHyphenWords() {
  const NO_BREAK_HYPHEN = "\u2011";
  const HYPHENATED_WORD_PATTERN = /([A-Za-z])-([A-Za-z])/g;
  ...
  const normalizeNode = (node) => {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      const normalized = currentValue.replace(HYPHENATED_WORD_PATTERN, `$1${NO_BREAK_HYPHEN}$2`);
    ...
    if (!(node instanceof Element) || EXCLUDED_TAGS.has(node.tagName)) {
      return;
    }
